...
public class SPSummaryCasinoV3 implements Serializable{
	...
	public List<SummaryCasinoV3> getSPSummaryCasinoV3(int Op, int Criterio, long FechaI, long FechaF, int EmployeeID,float valueVLT) throws Exception {
			...
			while (rs.next()) {
				...
				//if (!Aux.equals(AuxST)){
					list.add(new SummaryCasinoV3(
					    ...
					    rs.getString("Produttori"),
					    
					    //nuove modific
					    rs.getInt("ngiorni"),
					    rs.getInt("salaChiusa"),
					    rs.getDouble("jkContribution"),
					    rs.getFloat("vltSum"),
					    rs.getInt("M1"),
					    rs.getInt("M2")
